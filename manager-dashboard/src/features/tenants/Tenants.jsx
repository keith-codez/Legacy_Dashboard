...
function TenantList({
  ...
  /* ---------------- NAVIGATION LAYER (CRITICAL FIX) ---------------- */
  const goToTenant = (tenantId) => {
    if (onViewTenant) return onViewTenant(tenantId);
  ...
  const editTenant = (tenantId) => {
    if (onEditTenant) return onEditTenant(tenantId);

    navigate(`/manager/tenants/${tenantId}/edit`);
  };

  const addTenant = () => {
    if (onAddTenant) return onAddTenant();

    navigate("/manager/tenants/new");
  };
